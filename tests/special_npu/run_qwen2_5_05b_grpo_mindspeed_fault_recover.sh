...
TOKEN_CKPT=${MODEL_PATH:-${HOME}/tokens_ckpt/tokens.pt}
...
python3 -m recipe.fault_recover.main_ppo --config-path=config \
    ...
    fault_manager.max_reschedule_times=0 \
    fault_manager.max_rebuild_times=0 \
    fault_manager.tokens_save_file=${TOKEN_CKPT} \
